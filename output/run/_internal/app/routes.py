...
def register_user():
    if request.method == 'POST':
        ...
        return redirect(url_for('main.register_user'))  # Redirigir al login después de registrar

    return render_template('register.html', page_title='Registrar usuario')
...
def register_cliente_dispositivo():
    if request.method == 'POST':
        ...
        if not (nombre and apellidos and nit and correo and direccion and telefono ):
            flash('Todos los campos del cliente son obligatorios.', 'danger')
            return make_response(redirect(url_for('main.register_cliente_dispositivo')))
        if not (marca and modelo and detalles and fechaIngreso and IMEI and per_recibe and clave_tel and abono):
            flash('Todos los campos del dispositivo son obligatorios.', 'danger')
            return make_response(redirect(url_for('main.register_cliente_dispositivo')))

        # Guardar cliente y dispositivo en la base de datos
        nuevo_cliente = Cliente(nombre=nombre, apellidos=apellidos, nit=nit, correo=correo, direccion=direccion, telefono=telefono, )
        db.session.add(nuevo_cliente)
        db.session.commit()

        dispositivo = Dispositivo(idCliente=nuevo_cliente.idCliente, marca=marca, modelo=modelo, detalles=detalles, fechaIngreso=fechaIngreso,
                                  imei=IMEI, per_recibe=per_recibe, clave_tel=clave_tel, abono=abono)
...
def inicio():
    hoy = date.today()
    clientes_hoy = Cliente.query.filter_by(idCliente=hoy).all()
    dispositivos_hoy = Dispositivo.query.filter_by(fechaIngreso=hoy).all()
...
def buscar():
    ...
    if cliente_results:
        for cliente in cliente_results:
            dispositivos = Dispositivo.query.filter_by(idCliente=cliente.idCliente).all()
            dispositivo_results.extend(dispositivos)  # Agregar los dispositivos encontrados a la lista

    return render_template('resultado_busqueda.html', page_title='Buscar', 
                           cliente_results=cliente_results, 
                           dispositivo_results=dispositivo_results)
